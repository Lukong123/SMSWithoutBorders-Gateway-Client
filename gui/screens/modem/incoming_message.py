...
class IncomingMessageWindow(Gtk.Box):
    def __init__(self, modem_name, modem_handler, messaging):
        ...
        self.message_ui(self.incoming_messages)
    ...
    def message_ui(self, messages):
        ...
        # self.container_main.foreach(lambda child: self.container_main.remove(child))
        sorted_messages = sorted(
            messages, 
            key=lambda msg: datetime.strptime(
                self.format_timestamp(msg.new_received_message()[2]), 
                "%Y-%m-%dT%H:%M:%S%z"
            ), 
            reverse=True
        )
        for message in sorted_messages:
            ...
            # values
            number_value = str(sender_number)
            text_value = str(message_text)
            ...
            self.container_main.pack_start(message_box, False, False, 0)
            self.container_main.reorder_child(message_box, 0)
            
        # self.container_main.queue_draw()
    ...
    def on_delete_button_clicked(self, widget, message_path):
        if self.messaging:  # Check if messaging is available
            self.messaging.__delete_sms__(message_path)
        else:
            print("Messaging instance is not set.")
            print(f"alleged message path {message_path}")


    def refresh_messages(self, messages):
        """Refreshes the message list UI with updated messages."""
        # Clear existing message rows
        for child in self.message_list.get_children():
            self.message_list.remove(child)

        # Re-display all messages after clearing
    ...
    def format_timestamp(self, timestamp):
        """Ensure the timestamp has a timezone offset in full format (e.g., +01:00)."""
        logging.debug(f"Original timestamp: {timestamp}")

        # Check if the timezone part is short (e.g., +01) and add ":00" if necessary
        if len(timestamp) > 6 and (timestamp[-3] == '+' or timestamp[-3] == '-') and len(timestamp[-3:]) == 3:
            timestamp += ":00"

        logging.debug(f"Formatted timestamp: {timestamp}")
        return timestamp
    ...
    def display_all_messages(self, messages):
        ...
        for message in messages:
            ...
            self.message_text, self.sender_number, self.timestamp = message.new_received_message()
