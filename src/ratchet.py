...
if __name__ == "__main__":
    ...
    print(f" public key original: {base64.b64encode(bob_public_key_original)}")
    ...
    print(f"alice_encrypt: {alice_ciphertext}")

    s_header = header.serialize()
    print(f"header serialized: {s_header}")
    a_header1 = HEADERS.deserialize(s_header)
    print(f"headre deserialized: {a_header1}")
    assert header == a_header1

    bob_state = States()
    print(f"bob state: {bob_state}")
    bob1 = x25519("bob_keys.db")
    ...
    print(f"printing bob plaintext after decrypting: {bob_plaintext}")
